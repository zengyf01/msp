...
public class DataSourceServiceImpl implements DataSourceService {
    ...
    private boolean testMySQLConnection(DataSource ds) {
        String url = String.format("jdbc:mysql://%s:%d/%s?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useSSL=false&allowPublicKeyRetrieval=true",
            ds.getHost(), ds.getPort() != null ? ds.getPort() : 3306, ds.getDatabase());
        String user = resolveUser(ds);
        String pass = resolvePass(ds);
        try (Connection conn = DriverManager.getConnection(url, user, pass)) {
            return conn.isValid(5);
        } catch (Exception e) {
            System.err.println("MySQL连接失败: host=" + ds.getHost() + ", port=" + ds.getPort() + ", db=" + ds.getDatabase() + ", user=" + user + ", error=" + e.getMessage());
            return false;
        }
    }
